...
def phase1_hybrid_classify(question: str, messages: List[Dict], queries: List[Dict], turn_to_qids: Dict[int, List[str]], top_k: int, sent_window: int, max_snips: int, result_counts: Dict[str, int], qid_search_docs: Dict[str, str], turn_docs: Dict[str, str], visit_summary_hard_failures: Dict[str, List[Dict[str, Any]]], query_url_metadata: Dict[str, Dict[str, List[str]]], turn_doc_chunks: Dict[str, List[Dict[str, Any]]], turn_thinks: Dict[int, str], turn_visible_tool_message_indices: Dict[int, Optional[Set[int]]], keep_tool_result: int, turn_contexts: Dict[int, Dict[str, Any]]) -> Tuple[List[Dict], List[Dict], List[str]]:
    ...
    from .parsing import compute_visible_tool_message_indices, tokenize, tokenize_set
    ...
    for t in tqdm(range(1, max_turn + 1), desc='  Phase 1 (hybrid)'):
        ...
        visible_tool_message_indices = turn_visible_tool_message_indices.get(t, compute_visible_tool_message_indices(0, keep_tool_result))
        for qid in new_qids:
            q_text = qid_to_text.get(qid, '')
            ...
            for tok, st in token_status.items():
                if st == 'Q0_ONLY':
                    continue
                raw_hits = token_provenance_in_prior_docs(token=tok, prior_qs=prior_qs, qid_chunks=turn_doc_chunks, visible_tool_message_indices=visible_tool_message_indices, sent_window=sent_window, max_hits_total=max(80, top_k * max_snips * 10))
        ...
        per_turn_raw.append({'turn': t, 'soft_fail_anchor_rejection': soft_fail_anchor_rejection, 'keep_tool_result': keep_tool_result, 'visible_tool_message_indices': 'ALL' if visible_tool_message_indices is None else sorted(visible_tool_message_indices), 'raw_edges': raw_edges, 'no_source_found': no_src, 'mpsc_debug': mpsc_debug, 'prompt': prompt, 'per_query_payload': {qid: {'token_status': per_query_payload.get(qid, {}).get('token_status', {}), 'unsupported_new_tokens': per_query_payload.get(qid, {}).get('unsupported_new_tokens', []), 'unsupported_q0_seen_tokens': per_query_payload.get(qid, {}).get('unsupported_q0_seen_tokens', []), 'think_support': per_query_payload.get(qid, {}).get('think_support', {}), 'seen_latest_qid': per_query_payload.get(qid, {}).get('seen_latest_qid', {}), 'seen_first_qid': per_query_payload.get(qid, {}).get('seen_first_qid', {}), 'candidates_by_token': per_query_payload.get(qid, {}).get('candidates_by_token', {}), 'used_signals': sorted(per_query_payload.get(qid, {}).get('used_signals', []))} for qid in new_qids}, 'accepted_edges': [], 'rejected_edges': []})
        valid_ns: List[str] = []
        if no_src:
