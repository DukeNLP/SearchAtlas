...
def compute_visible_tool_message_indices(cutoff: int, keep_tool_result: int) -> Set[int]:
    """All-history mode retains all past results, never future results."""
    if keep_tool_result < -1:
        raise ValueError('keep_tool_result must be -1 or a non-negative integer')
    if cutoff <= 0 or keep_tool_result == 0:
        return set()
    start = 1 if keep_tool_result == -1 else max(1, cutoff - keep_tool_result + 1)
    return set(range(start, cutoff + 1))
...
def collect_visit_summary_hard_failures(raw_source: str, sanitized_block: str='') -> List[str]:
    """
    Detect failures produced by the visit-summary tool itself.
    """
    evidences: List[str] = []
    seen: Set[str] = set()
...
def render_search_json_response_for_provenance(obj: Dict[str, Any]) -> str:
    from .evidence import sanitize_tool_response_for_provenance
    lines: List[str] = []
    ...
    extracted = '\n'.join(lines).strip()
    return sanitize_tool_response_for_provenance(extracted or json.dumps(obj, ensure_ascii=False))
